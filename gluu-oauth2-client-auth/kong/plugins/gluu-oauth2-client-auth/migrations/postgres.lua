return {
    {
      ...
      CREATE TABLE IF NOT EXISTS gluu_oauth2_matrics(
        id uuid,
        client_id text,
        matrics json,
        created_at timestamp without time zone default (CURRENT_TIMESTAMP(0) at time zone 'utc'),
      ...
      DO $$
      BEGIN
        IF (SELECT to_regclass('gluu_oauth2_matrics_client_idx')) IS NULL THEN
          CREATE INDEX gluu_oauth2_matrics_client_idx ON gluu_oauth2_matrics(client_id);
        END IF;
      END$$;
    ]],
        down = [[
      DROP TABLE gluu_oauth2_client_auth_credentials;
      DROP TABLE gluu_oauth2_matrics;
    ]]
    }
}