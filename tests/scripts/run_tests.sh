...
function run_tests {
    ...
    /opt/katalon/Katalon_Studio_Linux_64-5.4/katalon --args -runMode=console -projectPath="$WORKSPACE/tests/katalon/Gluu Gateway/Gluu Gateway.prj" -reportFolder="$WORKSPACE/Reports" -reportFileName="report" -retry=0 -testSuitePath="$WORKSPACE/tests/katalon/Gluu Gateway/Test Suites/GG_tests" -browserType="Firefox (headless)" -email="$EMAIL" -password="$PASSWORD"
}
