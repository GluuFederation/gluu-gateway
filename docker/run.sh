...
if ! cat /opt/gluu-gateway/konga/config/local.js | grep "host: process.env.IP"; then
  ...
  cat /setup-gluu-gateway_template.py > /opt/gluu-gateway/setup/setup-gluu-gateway.py
  cd /opt/gluu-gateway/setup/
  python setup-gluu-gateway.py
  sed -i "64i\ \ host: process.env.IP || \'0.0.0.0\'," /opt/gluu-gateway/konga/config/local.js
  /etc/init.d/gluu-gateway restart
  echo "Gluu Gateway started!"
